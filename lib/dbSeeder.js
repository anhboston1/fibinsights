...
class DBSeeder {
    ...
    init() {
        ...
        mongoose.connection.db.listCollections({name: 'cashbuyers'})
        .next((err, collinfo) => {
            if (!collinfo) {
            console.log('Starting dbSeeder... cashBuyerSeed()');
            this.cashBuyerSeed();
        }
        });
        console.log('Done .. cashBuyerSeed()');
        mongoose.connection.db.listCollections({name: 'privatelenders'})
        .next((err, collinfo) => {
            if (!collinfo) {
            console.log('Starting dbSeeder... privatelenderSeed()');
            this.privateLenderSeed();
        }
    });
    ...
    privateLenderSeed(){
        console.log('Seeding privatelender data....');
        var fs = require('fs');
        var obj;
        fs.readFile(require('path').resolve(__dirname, 'privatelender.json'), 'utf8', function (err, data) {
            if (err){
                console.log(err);
            }
            else {
                ...
                for ( var i = 0; i < l; i++){
                    ...
                    var privatelender = new Privatelender({
                        ...
                        'fundsavailable': obj.data[i].fundsavailable,
                        'interestrate': obj.data[i].interestrate,
                        'lendingin': obj.data[i].lendingin
                    });
                    //console.log("before " + privatelender);
                    privatelender.save((err, cust) => {
                        if (err) {
                            console.log(err);
                        } else {
                            //console.log('inserted cashbuyer: ' + cust.name);
                            //console.log("after " + privatelender);
                    }
                });
    ...
    cashBuyerSeed()
    {
        console.log('Seeding cashbuyer data....');
        var fs = require('fs');
        var obj;
        fs.readFile(require('path').resolve(__dirname, 'cashbuyer.json'), 'utf8', function (err, data) {
            if (err){
                console.log(err);
            }
            else {
                ...
                for ( var i = 0; i < l; i++){
                    //console.log("obj.data[i].buyingin = ", obj.data[i].buyingin);
                    //console.log("obj.data[i].cashavailable = ", obj.data[i].cashavailable);


                    var cashbuyer = new Cashbuyer({
                        'name': obj.data[i].name,
                        'phone': obj.data[i].phone,
                        'buyingin': obj.data[i].buyingin,
                        'buyingcriteria': obj.data[i].buyingcriteria,
                        'cashavailable': obj.data[i].cashavailable
                    });
                    console.log("before " + cashbuyer);
                    cashbuyer.save((err, cust) => {
                        if (err) {
                            console.log(err);
                        } else {
                            //console.log('inserted cashbuyer: ' + cust.name);
                            console.log("after " + cashbuyer);
                        }
                    });
